...
OH는 수간 대비 상한 검증 비활성화. N블록 간격·휴식 앵커 간 근무일수 등은 검증 전용(간격 경고는 최소 5일).
...
def solve_schedule_cpsat(
    ...
    if status not in (cp_model.OPTIMAL, cp_model.FEASIBLE):
        if status == cp_model.INFEASIBLE:
            msg = 'CP-SAT: INFEASIBLE — model.Add 제약을 동시에 만족하는 배정이 없습니다.'
        elif status == cp_model.UNKNOWN:
            msg = 'CP-SAT: UNKNOWN — 시간 내 해를 찾지 못했습니다.'
        else:
            msg = f'CP-SAT: 풀이 실패 (status={status}).'
    ...
    if err_n == 0 and warn_n == 0:
        return sched, True, 'FEASIBLE_CP_SAT (검증 통과)'
    ...
    return sched, True, f'FEASIBLE_CP_SAT — 주의사항과 함께 생성됨.{tail}'
