# -*- coding: utf-8 -*-
"""
OR-Tools CP-SAT 근무표 솔버 (백업 사본 · DutySolution N 규칙 설명용).

【N(야간) 절대·검증 규칙과의 정렬】
- 간호사당 월 N은 **최대 7회** (`app.N_ABS_MAX`).
- N은 **2~3일 연속 블록**으로만 구성. **당월 말일**에만 **단독 N 1일** 예외 허용.
- 월 N이 정확히 7개일 때: 이월·당월을 합친 연속 N 구간은 **정확히 3개**이며,
  블록 길이 정렬이 **[2, 2, 3]** (즉 2-2-3, 2-3-2, 3-2-2) 또는 **[1, 3, 3]** 이고,
  후자일 때 길이 1인 블록은 **말일 단독**만 허용 (3-3-1 패턴).
- 서로 다른 N 블록 사이 **비N 일수 최소 5일** (검증·엔진 공통 기준, 본 파일 내 상수로도 참조 가능).

【솔버 개요】
- 하드: 일별 D 하한·상한, 고정 휴가, 일당 1시프트, 임산부 등 제약; 일별 E/N은 슬랙+가중으로 목표 충족.
- 경·공 등은 신청 칸 위주; 자동 배정 도메인은 주로 D/E/N/OF/OH.

이 파일은 UTF-8 인코딩이 깨졌던 시점의 **참고용 백업**이며, 실제 실행은 `schedule_cpsat.py`를 사용한다.
"""
...
# N 블록 간격이 하드 최소(5일)에 가깝거나 한 칸 더 타이트할 때 부과하던 소프트(과거 실험용)
_PENALTY_N_GAP_TIGHT_A = 25_000
_PENALTY_N_GAP_TIGHT_B = 20_000
_REWARD_N_OFOF = 3_800
_REWARD_N_OFE = 650
# 솔브 상한(초). 인원·패턴 탐색용; UNKNOWN 시 콜백 최선가해 사용 여부는 _CP_SAT_RETRY_ON_UNKNOWN 참조.
...
# 명시 D/E/N 신청과 배정 불일치 벌점(인원 슬랙보다 훨씬 작음).
_W_REQ_WORK_MATCH = 900_000
# 4연속 N·말일 외 단독 N 등 — 인원 슬랙보다 낮은 우선순위로만 지향(과거 소프트).
...
# 일별 D 하한·상한 하드 확정 후: E/N 슬랙·기타 패턴 소프트
...
def _carry_pond_r_last(carry_norm: dict, n: int) -> int | None:
    """?꾩썡 留덉?留????뱀썡 1??吏곸쟾)???닿쾶硫?1, 洹쇰Т硫?0, ?놁쑝硫?None."""
...
def _carry_pond_wr_first_next(
    ...
    """
    李⑥썡 1?? (w, r) 媛?0/1 ?먮뒗 遺덈챸 None.
    ?붾쭚 ?곗냽洹쇰Т(理쒖냼 2??쨌?곷떦 寃쎄퀎?먮쭔 ?ъ슜.
    """
...
def _add_ponddang_streak_soft(
    ...
    """
    ?곷떦쨌理쒖냼 2???곗냽 洹쇰Т: ?섎뱶 ???slack Bool + 紐⑹쟻 踰뚯젏.
    w+r_prev+r_next<=2+s, r_prev+w<=1+w_next+s ???됱냼 s=0 ?좏샇.
    """
    for n in regular:
...
def _add_weekly_equiv_linear(
    ...
    """
    app.weekly_of_equiv_satisfied(of, oh, no, m) 怨??숈튂???좏삎 ?쒖빟
    (BoolOr + OnlyEnforceIf 濡?OR 議곌굔 援ы쁽).
    """
    if m <= 0:
        return
    if m == 1:
...
def _build_n_block_boundary_maps(
    ...
    """媛?媛꾪샇?щ퀎 N 釉붾줉 '留먯씪 醫낅즺'쨌'泥レ씪 ?쒖옉' 0/1 ?쒗쁽 (蹂????踰뚮쭔 ?앹꽦)."""
...
def _add_n_block_min_gap_hard(
    ...
    """
    ?곗냽 N 釉붾줉 ?ъ씠 鍮껷 ?쇱닔 ??min_gap_days (gap = ?ㅼ쓬泥첥 ???댁쟾留륬 ??1).
    ?꾩썡 留?N ?닿퀬 ?뱀썡 1?쇱씠 ?곗냽???꾨땲硫? 泥??뱀썡 N? (min_gap+1)???댁쟾 湲덉?.
    """
    for n in regular:
...
def _add_n_block_gap_spread_soft(
    ...
    """
    鍮껷 媛꾧꺽???섎뱶(5??留뙿??쇰쭔???곗냽 釉붾줉 ?띿뿉 踰뚯젏 Bool(留먯씪 e1 ??泥レ씪 e2).
    ?섎뱶媛 ?꾨땶 ?뚰봽?몃줈留?7???댁긽 媛꾧꺽???좏샇.
    """
    off_a, off_b = 6, 7
    for n in regular:
        end_blk, start_blk = boundary_maps[n]
...
def _requests_clamped_to_nurses(requests: dict | None, num_nurses: int) -> dict | None:
    """遺???⑥쐞 ?몄썝(num_nurses) 諛뽰쓽 媛꾪샇???ㅻ뒗 ?쒖쇅(? 遺???쇱꽑 諛⑹?)."""
...
def _add_no_four_consecutive_n_soft(
    ...
    """4?곗냽 N쨌?꾩썡 N ?댁썡 寃쎄퀎: ?섎뱶 ????щ옓 Bool + 紐⑹쟻 踰뚯젏."""
    for n in regular:
...
def _off_of_or_oh(model: Any, x: dict, n: int, d: int, name: str) -> Any:
    """?대떦 ?쇱씠 OF ?먮뒗 OH(?대Т ?숇벑)?몄? 0/1. ????蹂???놁쑝硫??뺤닔 0留?諛섑솚(BoolVar ?꾨떂)."""
    has_of = (n, d, 'OF') in x
    has_oh = (n, d, 'OH') in x
...
def _x_has_off_shift_keys(x: dict, n: int, d: int) -> bool:
    """?대떦 ?쇱뿉 OF/OH 寃곗젙蹂?섍? ?덈뒗吏(?뚯씠??dict留?寃?? OR-Tools ?앷낵 鍮꾧탳?섏? ?딆쓬)."""
...
def _add_n_of_d_carry_hard(
    ...
    """?꾩썡 留?N 吏곹썑 ?뱀썡 1???대Т(OF/OH)쨌2??D/EDU 蹂듦? 湲덉? (?붾궡 N-?대Т-D? ?숈씪)."""
    if num_days < 2:
        return
    for n in regular:
...
def _collect_n_recovery_reward_vars(
    ...
    """
    N ???댄? ?대Т(OF/OH)쨌李⑥꽑 N-?대Т-E 吏?? 紐⑹쟻 Minimize?먯꽌 鍮쇰㈃ 理쒕???
    """
    r_ofof: list[Any] = []
    r_ofe: list[Any] = []
    for n in regular:
...
def _streak_terms_for_month_day(
    ...
    """?곗냽洹쇰Т(寃利?STREAK_WORK_SHIFTS)??????곸닔??+ BoolVar 由ъ뒪??"""
    if (n, dn) in locked:
...
def _restore_sched_from_values(
    ...
    """BoolVar ?좊떦 留듭뿉???쇰컲 媛꾪샇???쇰퀎 ?쒗봽??蹂듭썝."""
...
def _sched_fallback_requests_of(
    ...
    """?붾쾭媛 ?대? 紐??대㈃: 紐낆떆 ?좎껌 ?좎?, 鍮?移몄? ?꾩떆 OF(?붾쾭 誘몄쟻???쒖떆쨌?섍린 諛곗튂)."""
...
def solve_schedule_cpsat(
    ...
    """
    ?몄썝 ?곗꽑(?쇰퀎 E/N ?щ옓 理쒖냼)쨌D ?섑븳/?곹븳 ?섎뱶쨌紐낆떆 ?닿? 怨좎젙쨌D/E/N ?좎껌? ?뚰봽??
    ?곗냽 N ???⑦꽩? 踰뚯젏留? ?곹븳 ?쒓컙 ??理쒖꽑 媛?는톃NKNOWN ??肄쒕갚 理쒖쟻媛??ъ슜, ?ㅽ뙣 ???좎껌+?꾩떆OF ?대갚.
    """
    ...
    if num_nurses < 2:
        return None, False, 'CP-SAT: 가용 간호사가 부족합니다.', []
    ...
    # 단일 부서 인덱스 출처: 인덱스가 num_nurses 범위가 아닌 부서에서는 잘못된 값으로 간주하고 제외
    ...
    for ni in preg_set:
        ...
        for dnk, shv in ds.items():
            ...
            try:
                ...
                continue
            return None, False, (
                f'규칙: 임산부 {_names[ni]}는 야간(N) 신청·배정이 불가합니다.'
            ), []
    for ni, ds in req_norm.items():
        if not (0 <= ni < num_nurses):
            continue
        for dn, shv in ds.items():
            ...
            if shv == 'OH' and dni not in holiday_days:
                return None, False, (
                    'CP-SAT: OH는 공휴일 날짜 목록에 포함된 날에만 신청·배정할 수 있습니다. '
                    f'{dni}일은 공휴 목록에 없습니다.'
                ), []
    ...
    num_reg = len(regular)
    n_gap_suffix = f' (N 블록 간 최소 {_N_BLOCK_GAP_MIN_DAYS}일·목표·벌점 완화)'
    ...
    # 紐낆떆 ?닿?쨌怨좎젙移몃쭔 ?섎뱶 ?좉툑. 鍮덉뭏쨌D/E/N ?좎껌? 蹂??紐⑹쟻?먯꽌 ?좎껌 ?쇱튂???뚰봽??.
    for n in regular:
        for d in range(1, num_days + 1):
            rs = _req_shift_at(req_norm, n, d)
            if not rs:
    ...
    # N-(?ㅼ쓬??OF ?⑦꽩 吏?? d-1??N ?닿퀬 d??OF?대㈃ ?뚰봽???꾨컲(?좎껌 OF??d?쇱씠硫?踰뚯젏?? D/E ?ㅼ쓬 OF蹂대떎 ???좏샇?섍쾶)
    for n in regular:
    ...
    # ?쇰퀎 ?몄썝 ??E/N? 遺??理쒖냼(?댁긽 ?덉슜), D???섑븳쨌?곹븳(?묎툒??A1 ?됱씪 D=1 ??.
    for d in range(1, num_days + 1):
        day = days[d - 1]
        hsh = head.get(d) or ''
        need_e, need_n, (lo, hi) = app.daily_regular_staff_targets(
    ...
    # N-D쨌E吏곹썑湲덉?쨌N-OF-D쨌留먮떒 N ?????좎껌怨?異⑸룎 媛?ν븯誘濡?踰뚯젏 ?꾪솕
    for n in regular:
        for d in range(2, num_days + 1):
    ...
    # 3?곗냽 N 釉붾줉 吏곹썑 ?ㅼ쓬??OF/OH 沅뚯옣(4?곗냽 N? ???뚰봽??踰뚯젏?쇰줈 吏??.
    for n in regular:
    ...
    if val_map is None:
        ...
        if status == cp_model.INFEASIBLE:
            ...
            status_name = f'{status_name}·완화폴백(신청+임시OF)·진단:{_hint}'
    ...
    tail = '완화·목적N/A' if _fb_used else f'목적값={obj_val:.0f}'
    status_str = (
        f'CP-SAT [{status_name}] {tail}·탐색한도({_solve_budget_note})·'
        f'검증·오류 {err_n}·경고 {warn_n}건{n_gap_suffix}'
    )
